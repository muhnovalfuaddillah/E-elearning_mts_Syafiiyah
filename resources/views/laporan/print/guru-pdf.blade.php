...
<head>
    ...
    <title>Laporan Data Guru - MTs Syafiiyah</title>
    <!-- Google Fonts -->
...
<body class="p-4 md:p-8 print-padding">
    ...
    <div class="max-w-7xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 document-card">
        ...
        <div class="flex justify-between items-center mb-6 pb-3 border-b border-slate-200 text-[11px] text-slate-500 font-semibold">
            <p>Nomor: {{ $nomorSurat ?? str_pad(rand(1,999), 3, '0', STR_PAD_LEFT) . '/MTsS/GUR/' . \Carbon\Carbon::now()->format('m/Y') }}</p>
            <p>Sifat: Laporan Rutin</p>
        </div>

        <!-- Document Title -->
        <div class="text-center mb-6">
            <h3 class="font-extrabold text-base md:text-lg uppercase tracking-wider text-slate-800 underline">Laporan Rekapitulasi Data Guru Aktif</h3>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-5 rounded-2xl border border-slate-100">
            <div class="space-y-0.5 col-span-2">
                <span class="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Kategori</span>
                <p class="text-blue-600 font-bold text-sm">Seluruh Pendidik & Staf Aktif</p>
            </div>
            <div class="space-y-0.5">
                <span class="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Total Guru</span>
                <p class="text-slate-800 font-bold text-sm">{{ $gurus->count() }} Orang</p>
            </div>
            <div class="space-y-0.5">
            ...
            <table class="w-full text-xs md:text-sm">
                <thead>
                    <tr class="bg-blue-600 text-white">
                        ...
                        <th class="p-3 text-center font-bold w-36">NIP</th>
                        <th class="p-3 text-left font-bold">Nama Guru</th>
                        <th class="p-3 text-left font-bold w-56">Email</th>
                        <th class="p-3 text-center font-bold w-20">L/P</th>
                        <th class="p-3 text-left font-bold w-52">Mata Pelajaran</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($gurus as $index => $item)
                        <tr class="hover:bg-slate-50 transition-colors {{ $index % 2 === 1 ? 'bg-slate-50/50' : '' }}">
                            ...
                            <td class="p-3 text-center font-semibold text-slate-700">{{ $item->nip ?? '-' }}</td>
                            ...
                            <td class="p-3 font-medium text-slate-700">{{ $item->mapel ?? '-' }}</td>
        ...
        <div class="flex justify-end mb-10">
            <div class="bg-slate-50 border border-slate-100 rounded-xl px-5 py-2.5 text-xs font-bold text-slate-600">
                Total Guru Terdaftar: <span class="text-blue-600">{{ $gurus->count() }}</span> Orang
            </div>
        </div>
