...
<body class="p-4 md:p-8 print-padding">
    ...
    <div class="max-w-7xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 document-card">
        ...
        <div class="flex justify-between items-center mb-6 pb-3 border-b border-slate-200 text-[11px] text-slate-500 font-semibold">
            <p>Nomor: {{ $nomorSurat ?? str_pad(rand(1,999), 3, '0', STR_PAD_LEFT) . '/MTsS/NIL/' . \Carbon\Carbon::now()->format('m/Y') }}</p>
            <p>Sifat: Laporan Rutin</p>
        </div>

        <!-- Document Title -->
        <div class="text-center mb-6">
            <h3 class="font-extrabold text-base md:text-lg uppercase tracking-wider text-slate-800 underline">Laporan Rekapitulasi Nilai Akhir Mata Pelajaran</h3>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-5 rounded-2xl border border-slate-100">
            ...
            <div class="space-y-0.5">
                ...
                <p class="text-slate-800 font-bold text-sm">{{ $selectedMapel->nama_mapel }}</p>
            </div>
            <div class="space-y-0.5 col-span-2">
                <span class="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Guru Pengampu / Tanggal Cetak</span>
                <p class="text-slate-700 font-semibold text-xs leading-tight">
                    {{ $selectedMapel->guru ? $selectedMapel->guru->name : 'Belum ditentukan' }} <br class="no-print"> 
                    <span class="text-[10px] text-slate-400 font-normal">Cetak: {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</span>
                </p>
            </div>
        </div>
            ...
            <table class="w-full text-xs md:text-sm">
                <thead>
                    <tr class="bg-blue-600 text-white">
                        ...
                        <th class="p-3 text-center font-bold w-32">Rata Harian (40%)</th>
                        <th class="p-3 text-center font-bold w-24">UTS (30%)</th>
                        <th class="p-3 text-center font-bold w-24">Rank UTS</th>
                        <th class="p-3 text-center font-bold w-24">UAS (30%)</th>
                        <th class="p-3 text-center font-bold w-24">Rank UAS</th>
                        <th class="p-3 text-center font-bold w-24">Akhir</th>
                        <th class="p-3 text-center font-bold w-24">Rank Akhir</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($siswa as $index => $item)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors {{ $index % 2 === 1 ? 'bg-slate-50/50' : '' }}">
                            ...
                            <td class="p-3 text-center text-slate-600">{{ $harian !== null ? number_format($harian, 1) : '-' }}</td>
                            <td class="p-3 text-center text-slate-600">{{ $uts ?? '-' }}</td>
                            <td class="p-3 text-center font-medium text-slate-500">#{{ $rankUts }}</td>
                            <td class="p-3 text-center text-slate-600">{{ $uas ?? '-' }}</td>
                            <td class="p-3 text-center font-medium text-slate-500">#{{ $rankUas }}</td>
                            <td class="p-3 text-center text-blue-600 font-bold bg-blue-50/20">{{ $nilaiAkhir }}</td>
                            <td class="p-3 text-center text-blue-700 font-bold">#{{ $rankFinal }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="p-12 text-center text-slate-400 italic">
                                ...
                                Tidak ada data nilai di kelas ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
