...
<body class="p-4 md:p-8 print-padding">
    ...
    <div class="max-w-7xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 document-card">
        ...
        <div class="text-center mb-6">
            <h3 class="font-extrabold text-base md:text-lg uppercase tracking-wider text-slate-800 underline">Laporan Rekapitulasi Nilai Akhir Seluruh Mata Pelajaran</h3>
            ...
            <table class="w-full text-xs md:text-sm">
                <thead>
                    <tr class="bg-blue-600 text-white">
                        ...
                        <th class="p-3 text-center font-bold w-24">NIS</th>
                        <th class="p-3 text-left font-bold w-48">Nama Siswa</th>
                        @foreach($allMapels as $m)
                            <th class="p-3 text-center font-bold">{{ $m->nama_mapel }}</th>
                        @endforeach
                        <th class="p-3 text-center font-bold w-24">Rata-rata</th>
                        <th class="p-3 text-center font-bold w-24">Peringkat</th>
                    </tr>
                </thead>
                    ...
                    @forelse($siswa as $index => $item)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors {{ $index % 2 === 1 ? 'bg-slate-50/50' : '' }}">
                            ...
                            @foreach($allMapels as $m)
                                ...
                                @endphp
                                <td class="p-3 text-center text-slate-600">{{ $g ? ($g->nilai_akhir ?? '-') : '-' }}</td>
                            @endforeach
                            <td class="p-3 text-center text-blue-600 font-bold bg-blue-50/20">{{ round($avg, 1) }}</td>
                            <td class="p-3 text-center text-blue-700 font-bold">#{{ $rank }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ 5 + count($allMapels) }}" class="p-12 text-center text-slate-400 italic">
