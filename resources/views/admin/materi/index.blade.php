...
<div class="w-full px-4 md:px-6 py-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        <div class="luxury-card p-3 md:p-5">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-white/50 text-xs uppercase tracking-wider">Total Materi</p>
                    <h3 class="text-xl md:text-2xl font-bold stat-number mt-1 text-white">{{ $materi->total() }}</h3>
                    <p class="text-emerald-400 text-xs md:text-sm mt-2"><i class="fas fa-book"></i> Publik</p>
                </div>
                <div class="luxury-icon w-10 h-10 md:w-12 md:h-12">
                    <i class="fas fa-layer-group text-white text-base md:text-xl"></i>
                </div>
            </div>
        </div>
        ...
        <div class="luxury-card p-3 md:p-5">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-white/50 text-xs uppercase tracking-wider">Dokumen PDF</p>
                    <h3 class="text-xl md:text-2xl font-bold stat-number mt-1 text-white">
                        {{ \App\Models\Materi::where('tipe', 'pdf')->count() }}
                    </h3>
                </div>
        ...
        <div class="luxury-card p-3 md:p-5">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-white/50 text-xs uppercase tracking-wider">Presentasi PPT</p>
                    <h3 class="text-xl md:text-2xl font-bold stat-number mt-1 text-white">
                        {{ \App\Models\Materi::where('tipe', 'ppt')->count() }}
                    </h3>
                </div>
        ...
        <div class="luxury-card p-3 md:p-5">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-white/50 text-xs uppercase tracking-wider">Materi Video</p>
                    <h3 class="text-xl md:text-2xl font-bold stat-number mt-1 text-white">
                        {{ \App\Models\Materi::where('tipe', 'video')->count() }}
                    </h3>
                </div>
    ...
    <div class="luxury-card p-4 md:p-6 mb-6">
        <form method="GET" action="{{ route('admin.materi.index') }}" id="searchForm">
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.materi.index') }}" class="px-4 py-2 bg-white/5 rounded-lg text-white/70 hover:text-white text-sm flex items-center justify-center">
    ...
    <div class="luxury-card overflow-hidden">
        <div class="p-4 md:p-6 border-b border-white/10">
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                <div>
                    <h6 class="text-white font-semibold text-lg">Daftar Materi Pelajaran</h6>
                ...
                <div>
                    <button type="button" onclick="openCreateModal()" class="px-4 py-2 bg-emerald-500/20 rounded-lg text-emerald-400 text-sm flex items-center gap-2">
                        <i class="fas fa-plus"></i> Tambah Materi Pelajaran
                    </button>
                </div>
            </div>
        </div>
            ...
            <table class="w-full luxury-table min-w-[800px]">
                <thead class="border-b border-white/10 bg-white/5">
                    <tr>
                        ...
                        <th class="text-left p-3 md:p-4 text-white/60 text-xs font-semibold uppercase tracking-wider">Pengunggah</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td class="p-3 md:p-4 text-white/80 text-sm">
                            ...
                            <div class="text-white/30 text-xs uppercase">{{ $item->user->role ?? '' }}</div>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="p-12 text-center">
                            <div class="text-center">
                                ...
                                <p class="text-white/50">Belum ada materi pelajaran diterbitkan</p>
...
<script>
    ...
    function openEditModal(materi) {
        ...
        form.action = "{{ url('admin/materi') }}/" + materi.id;
    ...
    function confirmDelete(id, nama) {
        if (confirm(`Apakah Anda yakin ingin menghapus materi pelajaran "${nama}"?`)) {
            ...
            form.action = "{{ url('admin/materi') }}/" + id;
            form.submit();
        }
    }
