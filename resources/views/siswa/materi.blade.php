                                ...
                                <div class="luxury-card p-5 hover:border-blue-500/30 transition-all flex flex-col justify-between">
                                    ...
                                    <div class="mt-6 pt-4 border-t border-white/5">
                                        <div class="flex items-center justify-between text-xs text-white/40 mb-4">
                                            <span class="truncate pr-2"><i class="fas fa-chalkboard-teacher text-blue-400/80 mr-1"></i> {{ $item->guru->name ?? 'Guru Pengampu' }}</span>
                                            <span class="shrink-0"><i class="far fa-calendar-alt text-blue-400/80 mr-1"></i> {{ \Carbon\Carbon::parse($item->created_at)->format('d M Y') }}</span>
                                        </div>
                                        
                                        <div class="flex flex-col gap-2">
                                            @if($item->file_path)
                                                <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" download class="w-full py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-lg font-semibold text-xs transition flex items-center justify-center gap-1.5 border border-blue-500/20">
