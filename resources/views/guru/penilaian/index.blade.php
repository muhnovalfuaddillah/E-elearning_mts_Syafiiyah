        ...
        <form method="GET" action="{{ route('guru.penilaian.index') }}" id="filterForm">
            <div class="flex flex-col md:flex-row gap-4 items-end">
                <div class="flex-1 w-full">
                    ...
                    <select name="kelas_id" required class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:border-purple-500 focus:outline-none text-sm">
                ...
                <div class="flex-1 w-full">
                    ...
                    <select name="mapel_id" required class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:border-purple-500 focus:outline-none text-sm">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                    <div>
                        ...
                        <p class="text-white/40 text-xs md:text-sm">
                            Mapel: <strong class="text-purple-300">{{ \App\Models\MataPelajaran::find($selectedMapelId)->nama_mapel }}</strong> | 
                            Kelas: <strong class="text-blue-300">{{ \App\Models\Kelas::find($selectedKelasId)->kode_kelas }}</strong>
                        </p>
                    </div>
                    <div class="p-2.5 bg-purple-500/10 border border-purple-500/20 rounded-xl text-xs text-purple-400 max-w-sm">
                    ...
                    <table class="w-full luxury-table min-w-[950px]">
                        <thead class="border-b border-white/10 bg-white/5">
                            <tr>
                                ...
                                <th class="text-center p-2 text-white/60 text-xs font-semibold uppercase tracking-wider w-20 text-purple-300 bg-purple-500/5">Rata Harian (40%)</th>
                                ...
                                <th class="text-center p-2 text-white/60 text-xs font-semibold uppercase tracking-wider w-20 bg-pink-500/5 text-pink-300">Nilai Akhir</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($siswa as $index => $item)
                                ...
                                <tr class="border-b border-white/5 hover:bg-white/5">
                                    ...
                                    <td class="p-2">
                                        <span class="px-1.5 py-0.5 rounded-lg text-[10px] font-semibold bg-purple-500/20 text-purple-400">
                                            {{ $item->nis }}
                                        </span>
                                    </td>
                                    ...
                                    @for($i = 1; $i <= 6; $i++)
                                        ...
                                        <td class="p-2 text-center">
                                            <input type="number" 
                                                   ...
                                                   placeholder="-"
                                                   class="w-14 text-center px-1 py-1 bg-white/5 border border-white/10 rounded-lg text-white font-semibold focus:border-purple-500 focus:outline-none text-xs">
                                        </td>
                                    @endfor
                                    
                                    <!-- Rata-rata Nilai Harian (Read-only) -->
                                    <td class="p-2 text-center bg-purple-500/5">
                                    ...
                                    <td class="p-2 text-center">
                                        <input type="number" 
                                               ...
                                               min="0" max="100" step="0.01"
                                               placeholder="-"
                                               class="w-16 text-center px-1.5 py-1 bg-white/5 border border-white/10 rounded-lg text-white font-semibold focus:border-purple-500 focus:outline-none text-xs">
                                    </td>
                                    ...
                                    <td class="p-2 text-center bg-pink-500/5">
                                        @if($studentGrade && $studentGrade->nilai_akhir !== null)
                                            <span class="px-2 py-0.5 rounded-lg text-xs font-bold {{ $studentGrade->nilai_akhir >= 75 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400' }}">
        ...
        <div class="luxury-card p-8 md:p-12 text-center text-white/80">
            <div class="max-w-md mx-auto space-y-4">
                ...
                <div class="p-4 bg-white/5 rounded-xl border border-white/10 text-left text-xs space-y-2">
                    <p class="font-semibold text-purple-400"><i class="fas fa-info-circle"></i> Info Penilaian:</p>
