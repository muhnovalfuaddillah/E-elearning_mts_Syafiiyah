    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="luxury-card overflow-hidden">
            <div class="p-6 border-b border-white/10 bg-white/5">
                <h5 class="text-white font-bold text-lg"><i class="fas fa-edit text-blue-400 mr-2"></i> Edit Jurnal Mengajar</h5>
            ...
            <form action="{{ route('guru.jurnal.update', $jurnal->id) }}" method="POST" class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal', $jurnal->tanggal) }}" required class="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:border-blue-500 focus:outline-none text-sm">
                        @error('tanggal')
                            <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" min="1" name="pertemuan_ke" value="{{ old('pertemuan_ke', $jurnal->pertemuan_ke) }}" placeholder="Contoh: 1, 2, 3..." required class="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:border-blue-500 focus:outline-none text-sm font-semibold">
                        @error('pertemuan_ke')
                            <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea name="materi" rows="3" required placeholder="Tuliskan topik utama atau materi pelajaran yang dibahas..." class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white focus:border-blue-500 focus:outline-none text-sm leading-relaxed">{{ old('materi', $jurnal->materi) }}</textarea>
                    @error('materi')
                        <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="kegiatan" rows="4" required placeholder="Jelaskan secara ringkas aktivitas kelas (diskusi, demonstrasi, praktek kelompok, ceramah, dsb)..." class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white focus:border-blue-500 focus:outline-none text-sm leading-relaxed">{{ old('kegiatan', $jurnal->kegiatan) }}</textarea>
                    @error('kegiatan')
                        <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="3" placeholder="Tuliskan hambatan belajar, siswa yang tidak hadir, saran, atau hasil evaluasi mengajar..." class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white focus:border-blue-500 focus:outline-none text-sm leading-relaxed">{{ old('catatan', $jurnal->catatan) }}</textarea>
                    @error('catatan')
                        <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
