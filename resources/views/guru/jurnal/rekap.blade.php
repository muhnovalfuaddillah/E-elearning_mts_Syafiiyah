...
<head>
    ...
    <title>Rekap Jurnal Mengajar - {{ auth()->user()->name }}</title>
    <!-- Google Fonts -->
...
<body class="p-4 md:p-8 print-padding">
    ...
    <div class="max-w-5xl mx-auto mb-6 no-print flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        <a href="{{ route('guru.jurnal.index') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors">
    ...
    <div class="max-w-5xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 document-card">
        ...
        <div class="text-center mb-6">
            <h3 class="font-extrabold text-base md:text-lg uppercase tracking-wider text-slate-800 underline">Rekapitulasi Jurnal Mengajar Guru</h3>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-5 rounded-2xl border border-slate-100">
            <div class="space-y-0.5">
                <span class="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Nama Guru</span>
                <p class="text-slate-800 font-bold text-sm">{{ auth()->user()->name }}</p>
            </div>
            <div class="space-y-0.5">
            ...
            <table class="w-full text-xs md:text-sm">
                <thead>
                    <tr class="bg-blue-600 text-white">
                        ...
                        <th class="p-3.5 text-center font-bold w-20">Pertemuan</th>
                        <th class="p-3.5 text-left font-bold w-40">Mata Pelajaran</th>
                        <th class="p-3.5 text-left font-bold">Materi Pokok & Kegiatan</th>
                        <th class="p-3.5 text-left font-bold w-40">Catatan</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($jurnals as $index => $item)
                        <tr class="hover:bg-slate-50 transition-colors {{ $index % 2 === 1 ? 'bg-slate-50/50' : '' }}">
                            ...
                            <td class="p-3.5">
                                <p class="font-bold text-slate-800">{{ $item->mapel->nama_mapel }}</p>
                                <span class="text-[10px] text-slate-400 font-semibold">{{ $item->kelas->kode_kelas }}</span>
                            </td>
